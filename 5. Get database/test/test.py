...
firebase_admin.initialize_app(cred, {
    ...
    'storageBucket': 'medex-7ff53.appspot.com'  # Đảm bảo rằng bạn đã chỉ định đúng bucket
})
...
def download_image(image_url, user_id):
    ...
    local_image_path = os.path.join(images_folder, f"{user_id}.jpg")
    blob.download_to_filename(local_image_path)
    print(f"Đã tải hình ảnh của người dùng {user_id} về {local_image_path}")
        ...
        for user_id, user_data in users.items():
            if user_id not in fetched_users:
                ...
                if 'image_url' in user_data:
                    download_image(user_data['image_url'], user_id)
