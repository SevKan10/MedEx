...
def greet_user(file_path):
    try:
        ...
        if confirm == "True":
            voice = f"bạn đã được bác sĩ {doctor_name} xác nhận khám bệnh, bạn vui lòng để tay lên cảm biến nhịp tim để tiến hành đo."
            robotSpeak(voice)
        else:
            voice = f"bạn chưa được bác sĩ nào xác nhận khám bệnh, bạn vui lòng để tay lên cảm biến nhịp tim để tiến hành đo, sau đó đi đến quầy để được hỗ trợ thêm."
            robotSpeak(voice)
