...
open_eye = pygame.image.load('images/speak1.png')
closed_eye = pygame.image.load('images/speak2.png')
...
while running:
    ...
    open_time = random.uniform(1, 2)  # Thời gian ngẫu nhiên từ 2 đến 5 giây
    ...
    for _ in range(random.randint(1, 2)):  # Số lần chớp mắt ngẫu nhiên từ 1 đến 2 lần
        ...
        pygame.display.flip()
        time.sleep(random.uniform(0.05, 0.1))  # Thời gian nhắm mắt ngẫu nhiên từ 0.05 đến 0.2 giây

        # Hiển thị hình ảnh mở mắt
        screen.fill(background_color)
        screen.blit(open_eye, (eye_x, eye_y))
        pygame.display.flip()
        time.sleep(random.uniform(0.05, 0.1))  # Thời gian mở mắt ngẫu nhiên từ 0.05 đến 0.2 giây
